...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="changehands.entity.BabypageBean"%>
<%@page import="changehands.service.servlet.BabypageService"%>
...
<%
	int fID=-1;//得到分类得fID
	if(request.getParameter("fID")!=null){//fID不为空 则获取到
	...
	//宝贝的分页单独做  currentpage指当前页 recordpage指显示的数据
	...
	//调用BabypageService(Service层里的类)类里的方法
	BabypageService pService = new BabypageService();
	
	//通过getPageBean方法调用dao层的BabyDaoImpl类
	BabypageBean pBean = pService.getPageBean(currentpage, recordpage, UserID);
	
	//写入请求流
	request.setAttribute("fID", fID);
	request.setAttribute("pBean", pBean);
	request.setAttribute("productBeans", pBean.getBabyList());
...
<body>
	<div class="aside">
		<ul>
			...
			<li><a href="BabyPage.jsp?fID=8" target="indexFrameName" class="<c:if test="${requestScope.fID==8}">active_</c:if>">宝&nbsp;&nbsp;贝</a></li>
		</ul>
	</div>
	<div class="main">
		<ul>
			<c:forEach var="i" items="${requestScope.productBeans}">
				<li>
					<a href="BabyProductInfo.jsp?pID=${i.pID}&bID=${i.bID}" target="indexFrameName"><img src="imgs/${i.ShowPath}" width="200px"
	...
	<div class="fenye">
		<ul>			
			<c:choose>
				...
				<c:otherwise>
					<li><a href="BabyPage.jsp?fID=${requestScope.fID}">首&nbsp;页</a></li>
					<li><a
						href="BabyPage.jsp?currentpage=${requestScope.pBean.pre}&recordpage=${requestScope.pBean.numberpage}&fID=${requestScope.fID}">上一页</a></li>
				</c:otherwise>
			</c:choose>
			<!-- 当前页 -->
			...
			<c:choose>
				...
				<c:otherwise>
					<li><a
						href="BabyPage.jsp?currentpage=${requestScope.pBean.next}&recordpage=${requestScope.pBean.numberpage}&fID=${requestScope.fID}">下一页</a></li>
					<li><a
						href="BabyPage.jsp?currentpage=${requestScope.pBean.totalpage}&recordpage=${requestScope.pBean.numberpage}&fID=${requestScope.fID}">尾&nbsp;页</a></li>
				</c:otherwise>
			</c:choose>
		</ul>
	</div>
