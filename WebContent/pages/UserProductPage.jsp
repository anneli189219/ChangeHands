	...
	<div
		...
		<div style="margin: 0 auto; width: 580px; height: 600px;">
			<div
				...
				<p class="list_title">已发布的商品</p>
				<form action="UserProductSearch.jsp" method="post" style="text-align: right;" target="UserFrameName">
			...
			<table class="list_table_gridtable">
				...
				<c:forEach var="i" begin="${pPage*5-5}" end="${pPage*5-1}" items="${ProductList}">
					<tr>
						<td><img alt="封面图片" src="../imgs/${i.showPath}" width="40px"></td>
						<td>${i.tradeName}</td>
						<td>${i.describes}</td>
						<td>${i.price}</td>
						<td>${i.number}</td>
						<td>${g:gainString(i.addTime)}</td>
						<td><a href="ProductInfoPage.jsp?pID=${i.pID}">查看详情</a></td>
					</tr>
				</c:forEach>
			</table>
			<div class="list_page">
				<a href="../ProductPagingServlet?pPage=${pPage}&sFlag=0">首页</a> <a href="../ProductPagingServlet?pPage=${pPage}&sFlag=1">上一页</a><a href="#">${pPage}/${intPage}</a><a href="../ProductPagingServlet?pPage=${pPage}&sFlag=2">下一页</a>
				<a href="../ProductPagingServlet?pPage=${pPage}&sFlag=3">尾页</a> <a href="UserSalePage.jsp">查看销售记录</a>
			</div>
